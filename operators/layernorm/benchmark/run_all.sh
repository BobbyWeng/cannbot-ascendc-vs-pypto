...
ASCENDC_BIN="$ASCENDC_BUILD_DIR/layernorm_ascendc"
...
echo "============================================"
echo " LayerNorm Benchmark Runner"
echo "============================================"
...
if [ -n "${ASCEND_HOME_PATH:-}" ]; then
    ...
    echo "[WARN] ASCEND_HOME_PATH not set — proceeding anyway"
fi

# ------------------------------------------------------------------
# 2. Preflight checks — data, ascendc binary, pypto imports
# ------------------------------------------------------------------
echo ""
...
check_data() {
    local missing=0
    for b in "${BATCHES[@]}"; do
        if [ ! -f "$DATA_DIR/input_b${b}_${DTYPE}.bin" ]; then
            echo "[MISS] input_b${b}_${DTYPE}.bin"
            missing=1
        fi
        if [ ! -f "$DATA_DIR/reference_b${b}_${DTYPE}.bin" ]; then
            echo "[MISS] reference_b${b}_${DTYPE}.bin"
            missing=1
        fi
    done
    return $missing
...
" 2>&1 || echo "[WARN] PyPTO import check failed (may need torch_npu)"

# ------------------------------------------------------------------
# 3. Correctness verification (all three implementations)
# ------------------------------------------------------------------
echo ""
...
echo "[CORRECT] PyPTO..."
python3 "$PYPTO_DIR/tests/test_layernorm.py" 2>&1 || true
...
w = torch.from_numpy(np.fromfile(w_path, dtype=np.float16).reshape(norm_shape))
b = torch.from_numpy(np.fromfile(b_path, dtype=np.float16).reshape(norm_shape))
import torch_npu
torch.npu.set_device(device_id)
x_npu = x.npu(device_id)
w_npu = w.npu(device_id)
b_npu = b.npu(device_id)
...
echo " Summary"
echo "============================================"
echo " Operator  : layernorm"
echo " Shape     : [B, 256, 32]  B in {${BATCHES[*]}}"
echo " Dtype     : float16"
echo " Warmup    : $WARMUP"
echo " Loops     : $LOOPS"
echo " Repeat    : $REPEAT"
