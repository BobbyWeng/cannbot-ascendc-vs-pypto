# Reproducing Not (LogicalNot) Experiment
...
# Ascend C (all batches, random_mask case)
for b in 1 2 4 8 16 32 64; do
  ./ascendc/build/not_ascendc 0 $b 20 8192 1 1 1 data ascendc/build/output random_mask
  python3 data/generation_scripts/correctness.py ascendc/build/output/output_b${b}_random_mask.bin data/reference_b${b}_random_mask.bin $b
done
...
## Artifacts
All output files are stored within `operators/not/`.
