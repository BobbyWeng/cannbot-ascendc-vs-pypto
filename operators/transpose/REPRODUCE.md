...
```bash
cd operators/transpose/ascendc
...
```bash
# Torch baseline
python3 operators/transpose/torch/correctness.py --batch 1,2,4,8,16,32,64

# Ascend C (will print per-batch results)
python3 operators/transpose/ascendc/build/transpose_ascendc

# PyPTO
python3 -c "
import sys; sys.path.insert(0, 'operators/transpose/pyto/tests')
from test_transpose import test_transpose
test_transpose()
"
```
...
```bash
bash operators/transpose/benchmark/run_all.sh
```
