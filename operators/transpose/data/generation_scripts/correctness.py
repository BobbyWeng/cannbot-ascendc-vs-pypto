...
def check_correctness(output_path, reference_path, batch, verbose=True):
    ...
    expected_elements = batch * H_OUT * W_OUT
    if output.size != reference.size:
        return {
            "status": "FAIL",
            "error": f"Size mismatch: output={output.size}, reference={reference.size}, expected={expected_elements}"
        }

    if output.size != expected_elements:
        return {
            "status": "FAIL",
            "error": f"Expected {expected_elements} elements, got {output.size}"
        }
    ...
    # Check stride/contiguous consistency: verify Y[b,j,i] == X[b,i,j]
    if output.size == expected_elements:
        output_3d = output.reshape(batch, H_OUT, W_OUT)
    ...
    result = {
        ...
        "total_elements": int(expected_elements),
