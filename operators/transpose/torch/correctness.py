...
def load_batch_data(batch, device_id):
    ...
    ref = torch.from_numpy(np.fromfile(ref_path, dtype=np.float16).reshape([batch, W, H]))
...
def run_correctness(batch, device_id):
    ...
    # Materialized transpose: contiguous output
    output = x_npu.transpose(1, 2).contiguous()
    torch.npu.synchronize(device_id)

    # Verify shape and stride
    expected_shape = torch.Size([batch, W, H])
    if output.shape != expected_shape:
        return {"batch": batch, "status": "FAIL", "error": f"Shape: expected {expected_shape}, got {output.shape}"}

    # Verify contiguity
    if not output.is_contiguous():
        return {"batch": batch, "status": "FAIL", "error": "Output is not contiguous"}

    result = check_correctness(output, ref_npu, label=f"B={batch}")
    result["batch"] = batch
    result["shape"] = [batch, W, H]
    result["dtype"] = "float16"
    result["contiguous"] = output.is_contiguous()
    result["stride"] = list(output.stride())
    return result


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Torch baseline correctness for Transpose")
    ...
    for b in batches:
        ...
        if status == "PASS":
            print(f"  B={b}: PASS (contiguous={result.get('contiguous')}, stride={result.get('stride')})")
        elif status == "SKIP":
            print(f"  B={b}: SKIP ({result.get('error', '')})")
        else:
            print(f"  B={b}: FAIL (mismatches={result.get('bitwise_mismatch_count', '?')}, error={result.get('error', '')})")
            all_pass = False
