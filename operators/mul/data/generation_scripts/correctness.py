...
def check_correctness(output_path, reference_path, batch, verbose=True):
    ...
    total_elements = int(np.prod([batch] + SHAPE_TEMPLATE))
    ...
    # For Mul: signed-zero mismatches are NOT exempted
    # +0 (0x0000) vs -0 (0x8000)
    ...
    overflow_mask = np.isinf(output) & ~np.isinf(reference)
    underflow_mask = (output == 0.0) & (reference != 0.0) & ~np.isnan(reference) & ~np.isinf(reference)
    # More refined underflow: subnormal output that should be non-zero
    underflow_subnormal = (np.abs(output) < 6e-8) & (np.abs(reference) > 1e-5) & ~np.isinf(reference) & ~np.isnan(reference)
    ...
    result = {
        "status": "PASS" if (bitwise_equal or signed_zero_mismatch_count == bitwise_mismatch_count) else "FAIL",
        "strict_bitwise": bitwise_equal,
        ...
        "underflow_count": int(np.sum(underflow_mask)),
        "underflow_subnormal_count": int(np.sum(underflow_subnormal)),
    ...
    if verbose:
        ...
        print(f"    Strict bitwise: {result['strict_bitwise']}")
...
def check_all_batches(output_dir, data_dir, batches, variant_name):
    ...
    for b in batches:
        ...
        if result["status"] != "PASS":
            all_pass = False
            print(f"  [{variant_name}] b={b}: FAILED ({result['bitwise_mismatch_count']} mismatches, {result['signed_zero_mismatch_count']} sz)")
        else:
