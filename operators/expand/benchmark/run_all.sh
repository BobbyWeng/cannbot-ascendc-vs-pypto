...
ASCENDC_BIN="$ASCENDC_BUILD_DIR/expand_ascendc"
TORCH_DIR="$OPER_DIR/torch"
PYPTO_DIR="$OPER_DIR/pyto"
...
echo "[CORRECT] PyPTO..."
python3 "$PYPTO_DIR/tests/test_expand.py" 2>&1 || true
...
x = torch.from_numpy(np.fromfile(in_path, dtype=np.float16).reshape(batch, 256, 1))
import torch_npu
torch.npu.set_device(device_id)
x_npu = x.npu(device_id)

for _ in range(warmup):
    _ = expand_wrapper(x_npu)
...
echo " Summary"
echo "============================================"
echo " Operator  : expand"
echo " Shape     : [B, 256, 384]  B in {${BATCHES[*]}}"
echo " Dtype     : float16"
echo " Warmup    : $WARMUP"
echo " Loops     : $LOOPS"
echo " Repeat    : $REPEAT"
