...
def generate_fp16_input(batch, output_path, rng_state=None):
    ...
    specials.extend([-1e-3] * 16)
    specials.extend(float('nan') for _ in range(8))
    specials.extend(float('inf') for _ in range(8))
    specials.extend(float('-inf') for _ in range(8))
    rem = special_count - len(specials)
    if rem > 0:
        specials.extend(rng.uniform(-10, 10, rem).tolist())
