...
def check_all(output_dir, data_dir):
    ...
    for b in BATCHES:
        for variant in VARIANTS:
            out_path = os.path.join(output_dir, f"output_b{b}_{variant}.bin")
            ref_path = os.path.join(data_dir, f"reference_b{b}_{variant}.bin")
            key = f"b{b}_{variant}"
...
if __name__ == "__main__":
    ...
    print("OR operator correctness check:")
