#!/bin/bash
# Unified benchmark runner for Add (4-input)
set -euo pipefail
...
if [ ! -f "$BUILD_DIR/add_ascendc" ]; then
...
for b in $BATCHES; do
  ...
  msprof --output="$REPORT_RAW/ascendc_b${b}" \
    --ascendcl=on --ai-core=on --task-time=l0 \
    "$BUILD_DIR/add_ascendc" 0 "$b" 20 8192 "$WARMUP" "$LOOPS" "$REPEAT" "$DATA_DIR" "$BUILD_DIR/output"
  python3 "$OP_DIR/benchmark/parse_profiler.py" \
...
x3=torch.randn(${b},256,384,dtype=torch.float16).npu()
x4=torch.randn(${b},256,384,dtype=torch.float16).npu()
for _ in range(${WARMUP}): add_4(x1,x2,x3,x4)
...
x3=torch.randn(${b},256,384,dtype=torch.float16).npu()
x4=torch.randn(${b},256,384,dtype=torch.float16).npu()
for _ in range(${LOOPS}): add_4(x1,x2,x3,x4)
torch.npu.synchronize()
"
  python3 "$OP_DIR/benchmark/parse_profiler.py" \
