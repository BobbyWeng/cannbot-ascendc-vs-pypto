...
def load_inputs(batch, device_id):
    ...
    return x1.npu(device_id), x2.npu(device_id), x3.npu(device_id), x4.npu(device_id)


def four_input_add(x1, x2, x3, x4):
    return ((x1 + x2) + x3) + x4


def run_benchmark(batch, warmup, loops, repeat, device_id):
    torch.npu.set_device(device_id)
    x1_npu, x2_npu, x3_npu, x4_npu = load_inputs(batch, device_id)

    for _ in range(warmup):
        _ = four_input_add(x1_npu, x2_npu, x3_npu, x4_npu)
    ...
    return {
        ...
        "latency_us": stats,
        "bytes_per_elem": 10,
        "total_bytes": total_elems(batch) * 2 * 5,
    }


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Torch baseline benchmark for 4-input Add")
    ...
    output = {"results": results, "framework": "torch", "operator": "add"}
