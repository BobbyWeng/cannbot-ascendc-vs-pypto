...
```bash
# Torch baseline
python3 operators/relu/torch/correctness.py --batch 1,2,4,8,16,32,64

# Ascend C (will print per-batch results)
python3 operators/relu/ascendc/build/relu_ascendc

# PyPTO
python3 -c "
import sys; sys.path.insert(0, 'operators/relu/pypto/tests')
from test_relu import test_relu
test_relu()
"
```
...
```bash
bash operators/relu/benchmark/run_all.sh
```
...
3. Comparison is valid only for the specific shape [B,12,256,32] FP16 on Ascend 910B.
